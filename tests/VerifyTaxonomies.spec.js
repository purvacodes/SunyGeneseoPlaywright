...
test('Get All the Taxonomies from Drupal', async ({ page }) => {
  ...
  await page.goto('http://drupal-geneseo-backup.ddev.site/admin/structure/taxonomy/manage/status/overview', { timeout: 60000, waitUntil: 'domcontentloaded' });

  while (true) {
    // Get taxonomies texts from current page
    const texts = await page.locator('.menu-item__link').allTextContents();
    drupalTaxonomies.push(...texts.map(t => t.trim()));
...
test('Get All the Taxonomies from wordPress', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Log In' }).click();
  await page.goto('https://dev-infostride-geneseo.pantheonsite.io/wp-admin/edit-tags.php?taxonomy=status_tax&post_type=personal_page');
...
//       await page.waitForTimeout(10000);
  ...
  wordPressTaxonomies.push(...texts.map(t => t.trim()));
...
//   return { missingInWP, extraInWP,extraInDr };
// };

// test('Compare Drupal vs wordPress Taxonomies from JSON', async () => {
//   const drupalTaxonomies = JSON.parse(fs.readFileSync('drupalTaxonomies.json', 'utf-8'));
//   const wpTaxonomies = JSON.parse(fs.readFileSync('wordPressTaxonomies.json', 'utf-8'));

//   const { missingInWP, extraInWP } = compareTaxonomies(drupalTaxonomies, wpTaxonomies);
